...
namespace PBRInject
{
    ...
    class Program
    {
        ...
        internal static readonly uint[,] ExpTable =
       {
        ...
        static UInt32 SendCommand(String Command, Boolean Wait)
        {
            if (tcpClient.Connected)
            {
                ...
                if (Wait)
                {
                    ...
                    Byte[] Reply = new Byte[tcpClient.Available];
                    tcpClient.GetStream().Read(Reply, 0, Reply.Length);
                    String[] ResultArray = Encoding.UTF8.GetString(Reply).Split(' ');
                    return Convert.ToUInt32(ResultArray[2].Substring(0, ResultArray[2].Length - 1));
                }
            }
            return 0;
        }
